...
from keras.layers import Input, Dense, Reshape, Flatten, Embedding, merge, Dropout, Conv2DTranspose, Conv2D
...
def build_generator(latent_size):
    ...
    # upsample to (7, 7, ...)  ceil((7-k+1)/s)=3   ====>  k=5
...
def build_discriminator():
    ...
    cnn.add(Conv2D(32, (3, 3), padding='same', strides=2,
                   input_shape=(28, 28, 1)))
    cnn.add(LeakyReLU())
    cnn.add(Dropout(0.3))

    cnn.add(Conv2D(64, (3, 3), padding='same', strides=1))
    cnn.add(LeakyReLU())
    cnn.add(Dropout(0.3))
