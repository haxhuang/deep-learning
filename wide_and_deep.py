...
def maybe_download(train_data, test_data):
    ...
    if not os.path.exists(train_data):
        ...
        df_train = pd.read_csv("http://mlr.cs.umass.edu/ml/machine-learning-databases/adult/train.csv",
    ...
    if not os.path.exists(test_data):
        ...
        df_test = pd.read_csv("http://mlr.cs.umass.edu/ml/machine-learning-databases/adult/test.csv",
                              names=COLUMNS, skipinitialspace=True, skiprows=1)
    else:
        df_test = pd.read_csv("data/test.csv", names=COLUMNS)
...
def wide_deep(df_train, df_test, wide_cols, x_cols, embedding_cols, cont_cols, method):
    ...
    wide_deep.fit(X_tr_wd, Y_tr_wd, epochs=300, batch_size=128)
...
if __name__ == '__main__':
    ...
    ap.add_argument("--test_data", type=str, default="data/test.csv")
